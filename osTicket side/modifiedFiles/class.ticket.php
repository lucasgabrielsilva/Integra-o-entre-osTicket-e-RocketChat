...
implements RestrictedAccess, Threadable, Searchable {
    ...
    function setStatusForChat($status, $thisstaff, $comments='', &$errors=array(), $set_closing_agent=true) {
        global $cfg;
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postMessage($vars, $origin='', $alerts=true, $flag=False) {
        global $cfg;
      ...
      if ($recipients && $recipients instanceof MailingList)
        ...
        if(!$flag){
            ExternalChatController::prepareMessageForChat($this, "M", $message->ht['id']);
            ExternalChatController::prepareMessageForChat($this, "MU", $message->ht['id']);
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        ExternalChatController::prepareMessageForChat($this, "R", $response->ht['id'], html_entity_decode($response));
        ExternalChatController::prepareMessageForChat($this, "RU", $response->ht['id'], html_entity_decode($response));
        return $response;
    }
    ...
    function postNote($vars, &$errors, $poster=false, $alert=true) {
        ...
        $params = array('note' => html_entity_decode($note),
            'posterName' => $note->ht['poster'],
            'postDate' => $note->ht['created']);
        
        ExternalChatController::prepareMessageForChat($this, "N", $note->ht['id'], $params);
        return $note;
    }
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        $message = $ticket->postMessage($vars , $origin, false, True);
    ...
    ExternalChatController::prepareMessageForChat($ticket, "C", $ticket->getLastMessage()->ht['id']);
    return $ticket;
