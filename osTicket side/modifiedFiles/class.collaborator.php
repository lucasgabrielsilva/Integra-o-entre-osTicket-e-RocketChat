...
implements EmailContact, ITicketUser {
    ...
    function getUserNameForChat(){
        $id = $this->getId();
        $sql = "select dbuseraccount.username 
            from ost_user as dbuser, ost_user_account as dbuseraccount, ost_thread_collaborator as dbthreadcollaborator
            where dbthreadcollaborator.id = ".$id." and dbthreadcollaborator.user_id = dbuser.id and dbuseraccount.user_id = dbuser.id;";
        $res = db_query($sql);
        $res = db_result($res);
        return $res;
    }
